export function BackgroundGradient() {
  return (
    <div className="fixed inset-0 -z-10 overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-[var(--background)] via-[var(--background)] to-[var(--muted)/20]" />

      {/* Radial gradients for depth - multiple layers */}
      <div className="absolute -top-40 -right-40 h-96 w-96 rounded-full bg-[var(--primary)/20] blur-3xl" />
      <div className="absolute -bottom-40 -left-40 h-96 w-96 rounded-full bg-[var(--secondary)/20] blur-3xl" />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[600px] w-[600px] rounded-full bg-[var(--primary)/10] blur-3xl" />

      {/* Additional accent gradients with subtle animation */}
      <div
        className="absolute top-1/4 right-1/4 h-64 w-64 rounded-full bg-[var(--primary)/15] blur-2xl animate-pulse"
        style={{ animationDuration: "4s" }}
      />
      <div
        className="absolute bottom-1/4 left-1/4 h-64 w-64 rounded-full bg-[var(--secondary)/15] blur-2xl animate-pulse"
...
export function HeroGradient() {
  return (
    <>
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 h-[500px] w-[800px] rounded-full bg-[var(--primary)/10] blur-3xl" />
    </>
  );
}
